...
// DCDC输出电压配置
static void DCDC_OutVolt(u32 vol)
{
    ...
    while(1)
	{
		ANAC->DC_CFG |= BIT(0);
		DelayNus(20);
		if(ANAC->ANAC_FLAG & ANAC_ANAC_FLAG_DCDC_VOK)
		{
			u8 ret = 1;
			u32 start = TIM3->CTVAL;
    		while((start -(int)TIM3->CTVAL) < 50)
			{
				if(!(ANAC->ANAC_FLAG & ANAC_ANAC_FLAG_DCDC_VOK))
				{
					ret = 0;
		...
		ANAC->DC_CFG &= ~(BIT(0));//关闭DC-DC
		DelayNus(10);
	}
}

/***打开PWM***/
static void BuzLed_Pwm_On(u32 per, u32 duty)
{
	// 参数检查
	if(per < 2 || per < duty*2)
		return;
	...
	// PWM信号源配置
	ANAC->DC_CFG &= ~ANAC_DCBZ_CFG_BZ_PWM_SEL;
	ANAC->DC_CFG |= ANAC_DCBZ_CFG_BZ_PWM_SEL_TIM4_TOG;
